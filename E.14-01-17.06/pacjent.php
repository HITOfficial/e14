    ...
    <div class="container">
        ...
        <div class="middle">
            ...
            <div class="right">
                ...
                $conn= mysqli_connect('localhost','root','','przychodnia') or die ('blad podczas laczenia z serwerem');
                $id = $_POST['id'];
                $zapytanie2= 'select id, imie, nazwisko, choroby_przewlekle, uczulenia from pacjenci where id = '.$id;
                $zapytanie2query = mysqli_query($conn,$zapytanie2);
                if(mysqli_num_rows($zapytanie1query)>0){
                    while($pacjenci= mysqli_fetch_array($zapytanie2query)){
                        echo '<p>Imię i nazwisko: '.$pacjenci["imie"].' '.$pacjenci["nazwisko"]. '</p>';
                        echo '<p>Choroby przewlekłe: '.$pacjenci["choroby_przewlekle"]. '</p>';
                        echo '<p>Uczulenia: '.$pacjenci["uczulenia"].'</p>';
                    }
                }
                mysqli_close($conn);
                ?>
